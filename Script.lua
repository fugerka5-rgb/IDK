...
-- ========= [ TAB: Player — Selective NoClip (optimized, range=2) ] =========
local PlayerTab = Tabs.Player or Window:AddTab({ Title = "Player", Icon = "ghost" })

local snc_on     = PlayerTab:CreateToggle("snc_on",    { Title = "Selective NoClip", Default = false })
local snc_hold   = PlayerTab:CreateToggle("snc_hold",  { Title = "Hold-to-clip (key B)", Default = true })
-- диапазон 1..10 и дефолт 2, как просил
local snc_range  = PlayerTab:CreateSlider("snc_range", { Title = "Scan range (studs)", Min = 1, Max = 10, Rounding = 0, Default = 2 })
local snc_limit  = PlayerTab:CreateSlider("snc_limit", { Title = "Max parts / tick",  Min = 30, Max = 400, Rounding = 0, Default = 120 })
local snc_tick   = PlayerTab:CreateSlider("snc_tick",  { Title = "Update rate (s)",   Min = 0.05, Max = 0.60, Rounding = 2, Default = 0.22 })

-- новые тумблеры
local snc_addcap = PlayerTab:CreateSlider("snc_addcap",{ Title = "Max new constraints / scan", Min = 5, Max = 200, Rounding = 0, Default = 40 })
local snc_eco    = PlayerTab:CreateToggle("snc_eco",   { Title = "Eco mode (adaptive tick)", Default = true })
...
-- активные NoCollisionConstraint: [envPart] = { [charPart] = NCC }
local activeNCC = {}
local function addNoCollide(envPart: BasePart)
    local perChar = activeNCC[envPart]; if not perChar then perChar = {}; activeNCC[envPart] = perChar end
    for _,cp in ipairs(getCharParts()) do
...
-- основной цикл (адаптивный тик)
task.spawn(function()
    local lastScan = 0
    while true do
        if isDown() and root and root.Parent then
            local baseTick = snc_tick.Value
            local eco = snc_eco.Value

            -- чем больше активных связей, тем реже скан
            local dynamicTick = baseTick
            if eco then
                local load = 0
                for _ in pairs(activeNCC) do load += 1 end
                dynamicTick = baseTick * (1 + math.clamp(load / 120, 0, 3))
            end

            if tick() - lastScan >= dynamicTick then
                lastScan = tick()

                -- радиус гарантированно >= 1 (важно на очень малых значениях)
                local near = getNearBoogaParts(root.Position, math.max(1, snc_range.Value), snc_limit.Value)
                local keep = {}
                local added = 0
                local addCap = snc_addcap.Value

                for _,part in ipairs(near) do
                    keep[part] = true
                    if not activeNCC[part] and added < addCap then
                        addNoCollide(part)
                        added += 1
                    end
                end

                for part,_ in pairs(activeNCC) do
                    if (not part.Parent) or (not keep[part]) then
                        removeNoCollideFor(part)
                    end
                end
            end
            ...
            if next(activeNCC) ~= nil then clearAllNCC() end
            task.wait(0.18)
        end
    end
end)
...
-- =========================
-- TAB: Bed Spawn (GUI + packets, анти-спам, короткий фокус)
-- =========================
do
    ...
    -- создаём таб (и запоминаем в Tabs, если таблица есть)
    local BedTab = (Tabs and Tabs.BedSpawn) or Window:AddTab({ Title = "Bed Spawn", Icon = "bed" })
    if Tabs then Tabs.BedSpawn = BedTab end

    -- UI-параметры
    local bs_auto   = BedTab:CreateToggle("bs_auto",   { Title="Auto-press when visible", Default=false })
    local bs_debug  = BedTab:CreateToggle("bs_dbg",    { Title="Debug log (F9)", Default=false })
    local bs_period = BedTab:CreateSlider("bs_period", { Title="Scan interval (s)", Min=0.5, Max=5, Rounding=1, Default=2 })
    local bs_focus  = BedTab:CreateToggle("bs_focus",  { Title="Use focus fallback (Return)", Default=true })
    local bs_cd     = BedTab:CreateSlider("bs_cd",     { Title="Cooldown after press (s)", Min=0.5, Max=10, Rounding=1, Default=3 })
    local bs_once   = BedTab:CreateToggle("bs_once",   { Title="Only once per open", Default=true })

    local function dprint(...) if bs_debug.Value then print("[BedSpawn]", ...) end end

    -- --- Поиск кнопки ---
    local function looksLikeBed(inst)
        local n = tostring(inst.Name or ""):lower()
        if n:find("bedbutton") then return true end
        if n == "bed" then return true end
        if n:find("bed") and n:find("spawn") then return true end
        if inst:IsA("TextButton") then
            local t = tostring(inst.Text or ""):lower()
            if (t:find("bed") and t:find("spawn")) or t == "bed" then return true end
        end
        return false
    end
    local function bubbleToButton(inst)
        local p = inst
        for _=1,6 do
    ...
    local function findBedButton()
        local pg = plr:FindFirstChild("PlayerGui"); if not pg then return end
        -- быстрый точный путь
        local direct = pg:FindFirstChild("SpawnGui")
        direct = direct and direct:FindFirstChild("Customization")
        direct = direct and direct:FindFirstChild("BedButton")
        if direct and (direct:IsA("TextButton") or direct:IsA("ImageButton")) then
            return direct, "Players."..plr.Name..".PlayerGui.SpawnGui.Customization.BedButton"
        end
        -- общий скан
        for _,inst in ipairs(pg:GetDescendants()) do
            if inst:IsA("TextButton") or inst:IsA("ImageButton") or inst:IsA("TextLabel") then
                if looksLikeBed(inst) then
                    local btn = (inst:IsA("TextButton") or inst:IsA("ImageButton")) and inst or bubbleToButton(inst)
                    if btn then return btn, btn:GetFullName() end
                end
            end
        end
    end

    -- --- Нажатие (Activate -> клик -> короткий фокус) ---
    local function pressGui(btn)
        if not (btn and btn.Parent) then return false end
        local pressed = false
        ...
        pressed = pressed or ok1
        task.wait(0.02)

        -- 2) клик мышью по центру
        ...
        -- 3) короткий фокус + Enter (чтобы сервер принял, но WASD не «залипли»)
        if bs_focus.Value then
            pcall(function()
            ...
            -- мгновенно чистим фокус и «отпускаем» основные клавиши
    ...
    -- --- Пакеты / ремоты на случай, если GUI не срабатывает ---
    local packets do
        local ok, mod = pcall(function()
            return require(RS:WaitForChild("Modules"):WaitForChild("Packets"))
        end)
        packets = ok and mod or {}
    end
    local function tryPacketSpawn()
        local tried, success = {}, false
        local names = {
            "RespawnAtBed","SpawnAtBed","BedSpawn","RespawnBed",
            "SetSpawnBed","SelectSpawnBed","SetSpawnToBed"
        }
        for _,k in ipairs(names) do
            local m = packets and packets[k]
            if m and m.send then
                local ok = pcall(function() m.send() end)
                tried[#tried+1] = "packets."..k..":"..tostring(ok)
                success = success or ok
            end
        end
        local evBed = RS:FindFirstChild("Events") and RS.Events:FindFirstChild("Bed")
        if evBed then
            local req = evBed:FindFirstChild("RequestBeds")
            if req and req:IsA("RemoteFunction") then
                local ok, list = pcall(function() return req:InvokeServer() end)
                dprint("RequestBeds ->", ok and ("ok, count="..tostring(#(list or {}))) or "fail")
                for _,k in ipairs(names) do
                    local m = packets and packets[k]
                    if m and m.send then
                        local ok2 = pcall(function() m.send() end)
                        tried[#tried+1] = "packets."..k..":"..tostring(ok2)
                        success = success or ok2
                    end
                end
            end
        end
        if not success then dprint("packet attempts:", table.concat(tried, " | ")) end
        return success
    end

    -- --- Общее действие ---
    local lastPressT    = 0
    local pressedForBtn = nil
    local lastSeenBtn   = nil
    local lastSeenTime  = 0

    local function pressBedSpawn()
        local btn, path = findBedButton()
        if btn then
            if lastSeenBtn ~= btn then pressedForBtn = nil end
            lastSeenBtn  = btn
            lastSeenTime = tick()

            local cooldown = (bs_cd and bs_cd.Value) or 3
            local tooSoon  = (tick() - lastPressT) < cooldown
            local already  = bs_once.Value and (pressedForBtn == btn)

            if not tooSoon and not already then
                dprint("press GUI:", path)
                if pressGui(btn) then
                    lastPressT    = tick()
                    pressedForBtn = btn
                    return true
                end
            ...
            dprint("button not found in PlayerGui")
        end
        -- серверный запасной путь
        return tryPacketSpawn()
    end

    -- Кнопка разового нажатия
    BedTab:CreateButton({
        Title = "Find & Press now",
        Callback = function()
            local ok = pressBedSpawn()
            Library:Notify({
                Title   = "Bed Spawn",
                Content = ok and "Триггер отправлен (GUI/packet)." or "Не удалось: ни кнопка, ни пакеты не сработали.",
                Duration = 3
            })
        end
    })
    ...
    task.spawn(function()
        while true do
            local t = bs_period.Value
            if bs_auto.Value then
                pcall(function()
                    local ok = pressBedSpawn()
                    if ok then dprint("auto pressed") end
                end)
            end
            -- если кнопка пропала — разрешим новое нажатие
            if lastSeenBtn and (tick() - lastSeenTime) > 1.0 then
                pressedForBtn = nil
                lastSeenBtn   = nil
            end
            task.wait(t > 0 and t or 2)
        end
    end)
end

----------------------------------------------------------------
-- TAB: Coin Press (авто-пресс монет из золота)
----------------------------------------------------------------
local Options = Library.Options

-- гарантируем itemtouseID и прописываем золото
itemtouseID = itemtouseID or {}
itemtouseID["Gold"] = 597   -- как просил

-- вкладка
Tabs.CoinPress = Window:AddTab({ Title = "Coin Press", Icon = "coins" })

-- элементы UI
local cp_toggle = Tabs.CoinPress:CreateToggle("AutoCoinPress", {
    Title = "Auto Coin Press",
    Default = false
})
local cp_range = Tabs.CoinPress:CreateSlider("CoinPressRange", {
    Title = "Range (studs)",
    Min = 10, Max = 150, Rounding = 0, Default = 50
})
local cp_item = Tabs.CoinPress:CreateDropdown("CoinPressItem", {
    Title = "Item to press",
    Values = { "Gold" }, -- можешь добавить сюда ещё ("Raw Gold" и т.п.)
    Default = "Gold"
})
local cp_debug = Tabs.CoinPress:CreateToggle("CoinPressDebug", {
    Title = "Debug prints (F9)",
    Default = false
})
...
local function getPresses(range)
    local Presses = {}
    local dep = workspace:FindFirstChild("Deployables")
    local rp = getRoot()
    if not dep or not rp then return Presses end

    for _, deployable in ipairs(dep:GetChildren()) do
        if deployable:IsA("Model") and deployable.Name == "Coin Press" then
            ...
            if entityid and ppart then
                local dist = (ppart.Position - rp.Position).Magnitude
                if dist <= range then
...
local function UseCoinPress(entityid, itemID)
...
-- основной цикл (логика как у Campfire)
task.spawn(function()
    while true do
        if not Options.AutoCoinPress.Value then
            task.wait(0.1)
        else
            local selectedName = Options.CoinPressItem.Value
            local itemID = (itemtouseID and itemtouseID[selectedName]) or 597
            local range = tonumber(Options.CoinPressRange.Value) or 50

            local presses = getPresses(range)

            if Options.CoinPressDebug.Value then
                print("[CoinPress] item:", selectedName, "id:", itemID, "presses:", #presses)
            end

            for _, p in ipairs(presses) do
                UseCoinPress(p.entityid, itemID)
            end
